...
function AddProduct(){
    ...
    if(exist.length > 0){
        ...
        if(parseInt(exist[0][2]) != gianhap.value || exist[0][3] != giaCongThem.value){
            products.splice(products.indexOf(exist[0][0]), 1);
            thanhtien_value -= parseInt(exist[0][1]) * (parseInt(exist[0][2]) + parseInt(exist[0][3]));
...
function RemoveProduct(idsp, mausac, dungluong){
    
    // let idsp = document.getElementById('input-id').value;
    console.log("Remove: " + idsp);
    var isExisted = products.filter(item => item[0]==idsp && item[4] == mausac && item[5] == dungluong);
                ...
                <div class='item-detail-product'>
                    ...
                    <div class='item-detail-product-3'>
                        <div class='action-item'>
                            <button onclick="Action('-', ${data.id}, '${data.mausac}', '${data.dungluong}')">-</button>
                            <p id='quantity-${data.id}'>${quantity}</p>
                            <button onclick="Action('+', ${data.id}, '${data.mausac}', '${data.dungluong}')">+</button>
                        </div>
                        <button class='btn-delete' onclick="RemoveProduct(${data.id}, '${data.mausac}', '${data.dungluong}')">Xóa</button>
                    </div>
                </div>
            </div>`;
...
function Action(str, idsp, mausac, dl){
    var quantitysp = document.getElementById('quantity-'+idsp);

    if(str == "+"){
        products.forEach(element => {
            if(element[0] == idsp && element[4] == mausac && element[5] == dl){
